...
enum RainfallColorScale {
    ...
    static let stops: [Stop] = [
        ...
        Stop(
            threshold: 0.1,
            color: Color(.sRGB, red: 0.6, green: 0.9, blue: 0.6, opacity: 1),
            label: "0.1"
        ),
        ...
        Stop(
            threshold: 50,
            color: Color(.sRGB, red: 0.8, green: 0.2, blue: 0.8, opacity: 1),
            label: "50"
        ),
    ...
    static func rgbaComponents(for mm: Double) -> (
        ...
        precondition(
            visible.count >= 2,
            "RainfallColorScale needs at least 2 visible stops"
        )
        ...
        let t = CGFloat(
            (mm - lower.threshold) / (upper.threshold - lower.threshold)
        )
        return (
